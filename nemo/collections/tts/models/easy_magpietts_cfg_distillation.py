...
class _DefaultParams:
    ...
    # Fraction of the ground-truth sequence length used as a cutoff for rollout rejection.
    rejection_thresholds: tuple[Optional[float], Optional[float]] = (0.6, 1.25)
    ...
    phonemes_loss_weight: float = 0.0
...
def _validate_configuration(cfg: DictConfig) -> None:
    ...
    if hasattr(cfg, "rejection_thresholds"):
        thresholds = cfg.get("rejectionthreshold")

        if thresholds[0] is not None and thresholds[0] > 1.0:
            raise ValueError("Lower rejection threshold must be <= 1.0 or `None`.")

        if thresholds[1] is not None and thresholds[1] < 1.0:
...
class _TeacherInferenceEngine:
    ...
    def _apply_rollout_upper_bound_rejection(
        ...
        state.truncated = state.truncated | should_reject
        ...
        for item_idx in torch.nonzero(should_reject, as_tuple=False).flatten().tolist():
            logging.info(
                f"Item {item_idx} rejected by upper length bound at streaming generation step "
    ...
    def infer_batch(
        ...
        phoneme_sampling_method: str = "argmax",
        rejection_thresholds: tuple[Optional[float], Optional[float]] = (None, None),
        rejection_weight: Optional[float] = None,
    ) -> _TeacherOutput:
        """TBD."""
        ...
        with torch.no_grad():
            ...
            while not state.finished.all() and len(state.pred_codes) < max_decoder_steps:
                ...
                if rejection_thresholds[1] is not None:
                    state = self._apply_rollout_upper_bound_rejection(
                        state=state,
                        batch=batch,
                        upper_rejection_threshold=rejection_thresholds[1],
                        rejection_weight=rejection_weight,
                    )
            ...
            if rejection_thresholds[0] is not None:
                teacher_output = self._apply_final_lower_bound_rejection(
                    teacher_output=teacher_output,
                    batch=batch,
                    lower_rejection_threshold=rejection_thresholds[0],
                    rejection_weight=rejection_weight,
                )
        return teacher_output
...
class EasyMagpieCFGDistillation(EasyMagpieTTSModel):
    ...
    def __init__(
        ...
        self._val_engine = _ValidationInferenceEngine(params=self._val_inference_params)
    ...
    def on_fit_start(self) -> None:
        ...
        self._val_engine.setup()

    def on_validation_start(self) -> None:
        """TBD."""
        super().on_validation_start()

        if self._val_engine.is_active and torch.cuda.is_available():
            self._val_engine.switch_device(device=self.device)

    def on_validation_end(self) -> None:
        """TBD."""
        super().on_validation_end()

        if self._val_engine.is_active:
            self._val_engine.switch_device("cpu")
    ...
    def _crop_phonemes(
        ...
        if teacher_output.lens_phonemes is None:
            raise ValueError(
                "Expected `teacher_output.lens_phonemes` to be available when phoneme-channel "
    ...
    def _process_batch_distillation(
        ...
        teacher_output = self._teacher_inference_engine.infer_batch(
            ...
            rejection_thresholds=self.rejection_thresholds if mode == "train" else (None, None),
    ...
    def _validate_batch_using_verifiers(
        ...
        transcripts = self._val_engine.transcribe_files(
            paths=paths.generated,
            languages=batch.get("languages"),
        )
        speaker_embeds = self._val_engine.verify_speakers(paths)
        utmos_scores = self._val_engine.get_utmos_scores(
        ...
        output = _compute_validation_metrics(
            ...
            device=self.device,
            use_multilingual_asr=self._val_engine.use_multilingual_asr,
        )
        return output
    ...
    def on_validation_epoch_end(self) -> None:
        ...
        if self._val_engine.use_multilingual_asr:
